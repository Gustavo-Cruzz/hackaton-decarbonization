...
export function MapView({
  ...
  return (
    <section className="glass shadow-panel rounded-[32px] p-6">
      <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">
            Basemap online por padrao, geometrias locais como fallback e drill-down demonstrativo por UF ativa.
          </p>
        </div>
      ...
      <div className="mb-4 grid gap-3 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-[24px] bg-[linear-gradient(135deg,rgba(19,53,75,0.96),rgba(15,118,110,0.92))] p-4 text-white">
          ...
          <div className="mt-2 flex flex-wrap items-end gap-3">
            <div>
              <div className="text-2xl font-semibold">{territoryLabel}</div>
      ...
      <div className="mb-4 grid gap-3 xl:grid-cols-[0.85fr_1.15fr]">
        <div className="surface-strong rounded-[24px] p-4">
          <div className="flex flex-wrap gap-2">
          ...
          <p className="mt-3 text-xs text-[var(--muted)]">
            Tema ativo: {activeThemeLabel}. O restante da explicacao fica na legenda e no painel lateral.
          </p>
        </div>
        ...
        <div className="surface-strong rounded-[24px] p-4">
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {activeLayers.map((layerKey) => (
              <span key={layerKey} className="rounded-full bg-[rgba(15,118,110,0.12)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--teal)]">
                {layerDefinitions.find((layer) => layer.key === layerKey)?.label ?? layerKey}
              </span>
            ))}
          </div>

          {municipalityStatus === "loading" ? <p className="mb-3 text-sm text-[var(--muted)]">Carregando municipios da UF ativa...</p> : null}
          {municipalityStatus === "unavailable" ? (
            <p className="mb-3 rounded-[18px] bg-[rgba(217,119,6,0.08)] p-3 text-sm text-[var(--foreground)]">
